...
/**
 * A Bayesian Skyline Plot implementation using a fixed-length array of segments for the tree intervals
 *
 *
 * @author Louis du Plessis
 * @date 2019/01/17
 */
public class SegmentArrayBSP extends TreeDistribution {
...
//            groupLastSegments;
    ...
    BSPSegment [] segments, storedSegments;
    ...
    public void initAndValidate() {
        ...
        segments = new BSPSegment[tree.getNodeCount()];
        storedSegments = new BSPSegment[tree.getNodeCount()];
        for (int i = 0; i < segments.length; i++) {
            segments[i] = new BSPSegment();
        }
     ...
     *
     */
    protected void collectSegmentTimes() {
        ...
        for (int i = 0; i < nodes.length; i++) {
            segments[i].setEnd(nodes[i].getHeight());
            segments[i].setSegmentType(nodes[i].isLeaf() ? SegmentType.SAMPLE : SegmentType.COALESCENT);

            //segments[i] = new BSPSegment(nodes[i].getHeight(),
        ...
        // Not exactly the same as above, but good enough for testing if this implementation is faster (still not fast enough)
        Arrays.sort(segments, (o1, o2) -> (Double.compare(o1.getEnd(),o2.getEnd())));

        ////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        // Sanity checks - not necessary here, probably can be removed
        // First segment should start at 0
        //if (segments[0].getStart() != 0)
        //    throw new IllegalArgumentException("First segment does not start at 0.");
        // Last segment should stop at tMRCA
        //if (segments[segments.length-1].getEnd() != tree.getRoot().getHeight())
    ...
    public void updateSegments() {
        ...
        //System.out.println("Updating segments!");


        // Collect the times and types of all segments
        if (treeChanged) collectSegmentTimes();
    ...
    public double getChangeTime(int i) {
        ...
        return segments[groupLastSegments[i]].getEnd();
    }*/
    ...
    @Override
    public void store() {
        //System.out.println("Called store!");
        //segmentsUpdated = false;
        System.arraycopy(cumulativeGroupSizes, 0, storedCumulativeGroupSizes, 0, cumulativeGroupSizes.length);
        //System.arraycopy(segments, 0, storedSegments, 0, segments.length);
        for (int i = 0; i < segments.length; i++) {
            storedSegments[i] = new BSPSegment(segments[i]);
        }
    ...
    @Override
    public void restore() {
        //System.out.println("Called restore!");
        // segmentsUpdated = false;
        ...
        BSPSegment [] tmp2 = storedSegments;
